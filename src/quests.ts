...
export async function handleQuests() {
  ...
  if (questProg.progressStatus != ProgressStatus.COMPLETED) {
    if (!query(questProg).isTaskCompleted(taskIds.asianHerb)) {
      //   if (!query(questProg).isStepCompleted(stepIds.asian1)) {
      let herb1 = new Herb(
        'models/kimkim.glb',
        { position: new Vector3(261.8, 18.15, 237.4) },
        taskIds.asianHerb,
        //   stepIds.asian1,
        client
      )
      ...
      //   if (!query(questProg).isStepCompleted(stepIds.asian2)) {
      let herb2 = new Herb(
        'models/kimkim.glb',
        { position: new Vector3(207, 10.5, 124) },
        taskIds.asianHerb,
        //   stepIds.asian2,
        client
      )
      //   }
      //   if (!query(questProg).isStepCompleted(stepIds.asian3)) {
      let herb3 = new Herb(
        'models/kimkim.glb',
        { position: new Vector3(159.5, 12.85, 100) },
        taskIds.asianHerb,
        //   stepIds.asian3,
        client
      )
